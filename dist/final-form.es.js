...
var keysCache = Object.create(null);
var keysRegex = /[.[\]]+/;
  ...
  if (!isValidArrayIndex(key)) {
    ...
    if (_result2 === undefined) {
      ...
      var _current = current;
        _current[key];
        var _final = _objectWithoutPropertiesLoose(_current, [key].map(_toPropertyKey));
      return _final;
    }
    // set result in key
...
var configOptions = ["debug", "initialValues", "keepDirtyOnReinitialize", "mutators", "onSubmit", "validate", "validateOnBlur", "callbackScheduler", "ignoreUnregister"];
...
function createForm(config) {
  ...
  var debug = config.debug,
    ...
    ignoreUnregister = config.ignoreUnregister;
  if (!onSubmit) {
  ...
  var renameField = function renameField(state, from, to) {
    if (state.fields[from]) {
      ...
      state.fields = Object.assign(Object.create(null), state.fields, (_Object$assign = {}, _Object$assign[to] = _extends({}, state.fields[from], {
        name: to,
      ...
      state.fieldSubscribers = Object.assign(Object.create(null), state.fieldSubscribers, (_Object$assign2 = {}, _Object$assign2[to] = state.fieldSubscribers[from], _Object$assign2));
  ...
  var runFieldLevelValidation = function runFieldLevelValidation(field, setError) {
    ...
    if (validators.length) {
      ...
      var fieldValidationKey = field.asyncValidationKey;
      validators.forEach(function (validator) {
        var errorOrPromise = validator(getIn(state.formState.values, field.name), state.formState.values, validator.length === 0 || validator.length === 3 ? publishFieldState(state.formState, field) : undefined);
        if (errorOrPromise && isPromise(errorOrPromise)) {
          // Track async validation with per-field counter
          field.asyncValidationCount++;
          field.validating = true;
          var fieldInstanceId = field.instanceId; // Capture stable instance ID
          var promise = errorOrPromise.then(function (error) {
            var currentField = state.fields[field.name];
  ...
  var runValidation = function runValidation(fieldChanged, callback) {
    ...
    var promise = Promise.all(promises).then(clearAsyncValidationPromise(asyncValidationPromiseKey))["catch"](function (error) {
      ...
      var forEachError = function forEachError(fn) {
        fieldKeys.forEach(function (name) {
          if (fields[name]) {
            ...
            var asyncRecordLevelError = afterAsync ? getIn(asyncRecordLevelErrors, name) : undefined;
            var errorFromParent = getIn(merged, name);
            var hasFieldLevelValidation = getValidators(safeFields[name]).length;
            var fieldLevelError = fieldLevelErrors[name];
            fn(name, hasFieldLevelValidation && fieldLevelError || validate && (asyncRecordLevelError || recordLevelError) || (!recordLevelError && !limitedFieldLevelValidation ? errorFromParent : undefined));
          }
        });
      };
  ...
  var calculateNextFormState = function calculateNextFormState() {
    ...
    if (!foundDirty) {
      ...
      var allKeys = new Set([].concat(Object.keys(formState.values || {}), Object.keys(formState.initialValues || {})));
      for (var _iterator = _createForOfIteratorHelperLoose(allKeys), _step; !(_step = _iterator()).done;) {
        var key = _step.value;
        if (!safeFields[key]) {
          var currentValue = (_a = formState.values) === null || _a === void 0 ? void 0 : _a[key];
          var initialValue = (_b = formState.initialValues) === null || _b === void 0 ? void 0 : _b[key];
  ...
  var api = {
    ...
    initialize: function initialize(data) {
      ...
      var savedDirtyValues = keepDirtyOnReinitialize ? Object.keys(safeFields).reduce(function (result, key) {
        ...
        if (!pristine) {
          ...
          var hasChildFields = Object.keys(safeFields).some(function (otherKey) {
            return otherKey !== key && (otherKey.startsWith(key + '[') || otherKey.startsWith(key + '.'));
          });
          if (!hasChildFields) {
      ...
      Object.keys(safeFields).forEach(function (key) {
        var field = safeFields[key];
        var currentValue = getIn(formState.values, key);
        var initialValue = getIn(formState.initialValues || {}, key);
        var pristine = field.isEqual(currentValue, initialValue);
    ...
    registerField: function registerField(name, subscriber, subscription, fieldConfig) {
      ...
      if (typeof field.blur !== 'function') field.blur = function () {
        return api.blur(name);
      };
      if (typeof field.change !== 'function') field.change = function (value) {
        return api.change(name, value);
      };
      if (typeof field.focus !== 'function') field.focus = function () {
        return api.focus(name);
      };
      field.isEqual = fieldConfig && fieldConfig.isEqual || state.fields[name] && state.fields[name].isEqual || tripleEquals;
      // Ensure instanceId and async validation counters exist (for fields created by mutators)
      field.instanceId = (_a = field.instanceId) !== null && _a !== void 0 ? _a : ++nextFieldInstanceId;
      field.asyncValidationCount = (_b = field.asyncValidationCount) !== null && _b !== void 0 ? _b : 0;
      field.asyncValidationKey = (_c = field.asyncValidationKey) !== null && _c !== void 0 ? _c : 0;
      ...
      return function () {
        ...
        // istanbul ignore next
        if (state.fields[name] && state.fields[name].validators) {
          // state.fields[name] may have been removed by a mutator (e.g., renameField #191)
          validatorRemoved = !!(state.fields[name].validators[index] && state.fields[name].validators[index]());
          delete state.fields[name].validators[index];
        }
        var hasFieldSubscribers = !!state.fieldSubscribers[name];
        if (hasFieldSubscribers && state.fieldSubscribers[name].entries) {
          // state.fieldSubscribers[name] may have been removed by a mutator (e.g., renameField #191)
          delete state.fieldSubscribers[name].entries[index];
        }
        var lastOne = hasFieldSubscribers && state.fieldSubscribers[name] && state.fieldSubscribers[name].entries && !Object.keys(state.fieldSubscribers[name].entries).length;
        if (lastOne) {
    ...
    resetFieldState: function resetFieldState(name) {
      var _a, _b, _c;
      var field = state.fields[name];
      state.fields[name] = _extends({}, field, {
        active: false,
        lastFieldState: undefined,
        modified: false,
        touched: false,
        ...
        instanceId: (_a = field.instanceId) !== null && _a !== void 0 ? _a : ++nextFieldInstanceId,
        asyncValidationCount: (_b = field.asyncValidationCount) !== null && _b !== void 0 ? _b : 0,
        asyncValidationKey: ((_c = field.asyncValidationKey) !== null && _c !== void 0 ? _c : 0) + 1,
        visited: false
      });
      runValidation(undefined, function () {
        notifyFieldListeners(undefined);
